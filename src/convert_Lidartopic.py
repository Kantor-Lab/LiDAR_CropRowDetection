...
def lidar_callback(msg, marker_pub):
    ...
    # ransac.fit(pc_array[:, :2], pc_array[:, 2])
    ...
    # print(A)
    # Perform Singular Value Decomposition (SVD) to solve for the plane coefficients
    # U, S, VT = svd(A)
    # normal = VT[-1, :-1]
    # d = VT[-1, -1]

    # Calculate the plane parameters (A, B, C, D) for the equation Ax + By + Cz + D = 0
    ...
    plane_marker.pose.position.z = np.mean(z)
    roll = np.arctan(b / d)
    pitch = np.arctan(a / d)
    yaw = 0  # You can adjust the yaw as needed
    q = tf.transformations.quaternion_from_euler(roll, pitch, yaw)
    # print(q)
    # q1 = tf.transformations.quaternion_from_euler(0, -57, 0)
    q1 = tf.transformations.quaternion_from_euler(0, -0.7, 0)
    # q1 = tf.transformations.quaternion_from_euler(0, -70, 0)
    ...
    # plane_marker.pose.position.z = -c / c  # Adjust the height based on the plane equation
    # plane_marker.pose.orientation.x = 0
    # plane_marker.pose.orientation.y = 0
    # plane_marker.pose.orientation.z = np.sin(np.arctan(b / a / -c))
    # plane_marker.pose.orientation.w = np.cos(np.arctan(b / a / -c))
    ...
    for point in pc2.read_points(msg, field_names=("x", "y", "z")):
        ...
        # Extract the x-coordinate of the point as an angle (modify this if your data encodes angles differently)
            # if a * point[0] + b * point[1] + c * point[2] + d > -1:
            if a * point[0] + b * point[1] + c * point[2] + d > 0.06:
            # if a * point[0] + b * point[1] + c * point[2] + d > 0.00:
