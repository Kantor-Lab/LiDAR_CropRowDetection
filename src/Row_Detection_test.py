...
def calculate_kmeans(msg, pc_array,robot_position, robot_orientation, initial_orientation, initial_rotation_matrix):
    ...
    for row in fitting_points:
        if swiped_lines % 2 == 1:
            ...
            #test 1
            # sorted_list = sorted(row, key=lambda x: x[1])
            # fitting_centroids.append(sorted_list[len(sorted_list//2 - 1):len(sorted_list//2 + 1)])

            #test 2
            global last_centroids
            distances = [(sublist, abs(sublist[1])) for sublist in row if len(sublist) > 1]
            sorted_distances = sorted(distances, key=lambda x: x[1])
            closest_elements = [sorted_distances[0][0], sorted_distances[1][0]] if len(sorted_distances) >= 2 else []
            ...
            if closest_elements:
                ...
                for i in range(2):
                    fitting_centroids.append(last_centroids[i][:2])
